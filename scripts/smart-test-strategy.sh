#!/bin/bash
# Smart Test Strategy - @buildproven/wanderpage
...
HIGH_RISK_FILES=$(git diff --name-only "$DIFF_RANGE" 2>/dev/null | grep -E "auth|payment|security|crypto|api/|pages/api/|app/api/|middleware" || true)
