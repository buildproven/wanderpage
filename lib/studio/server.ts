...
export function createStudioServer({
  ...
  async function route(request: IncomingMessage, response: ServerResponse) {
    ...
    if (!trustedRequest(request, boundPort)) {
      json(response, 403, { error: "Wanderpage Studio only accepts requests from its local interface." });
      return;
    }
    ...
    if (request.method === "POST" && url.pathname === "/api/jobs") {
      ...
      try {
        ...
        void executeJob(job, productionRunner);
        trimJobs(jobs);
        return;
      } catch (error) {
        json(response, 400, { error: message(error) });
        return;
      }
    }
  ...
  async function executeJob(job: StudioJob, jobRunner: StudioRunner) {
    ...
    try {
      ...
      update(job, "failed", job.progress.progress, job.error);
    }
  }
  ...
  return {
    async start() {
      ...
      if (!address || typeof address === "string") throw new Error("Wanderpage Studio could not bind a local port");
      boundPort = address.port;
      return `http://127.0.0.1:${boundPort}/studio`;
    },
    async stop() {
...
function createJob(request: StudioJobRequest): StudioJob {
  ...
  return {
    ...
    updatedAt: now,
    request,
    progress: { stage: "queued", progress: 0, message: "Waiting to begin", at: now },
  };
}
function update(job: StudioJob, status: StudioJob["status"], progress: number, messageText: string, stage: string = status) {
  const now = new Date().toISOString();
  job.status = status;
  job.updatedAt = now;
  job.progress = { stage, progress: Math.round(progress), message: messageText, at: now };
}
...
async function readJson(request: IncomingMessage) {
  ...
  for await (const chunk of request) {
    body += String(chunk);
    if (body.length > 65_536) throw new Error("Request is too large.");
  }
  return JSON.parse(body || "{}");
}
function trustedRequest(request: IncomingMessage, port: number) {
  const host = request.headers.host ?? "";
  if (!new RegExp(`^(127\\.0\\.0\\.1|localhost):${port}$`).test(host)) return false;
...
async function serveFile(file: string, response: ServerResponse, head = false) {
  try {
    ...
    response.writeHead(200, { "Content-Type": mime(file), "Content-Length": metadata.size });
...
async function chooseFolder() {
  if (process.platform === "darwin") {
    const { stdout } = await execute("osascript", [
      "-e",
      'POSIX path of (choose folder with prompt "Choose the folder containing your trip photos")',
    ]);
    return stdout.trim().replace(/\/$/, "");
  }
  if (process.platform === "linux") {
    const { stdout } = await execute("zenity", ["--file-selection", "--directory", "--title=Choose your trip photo folder"]);
    return stdout.trim();
  }
