...
export async function publishPhoto(photo: PhotoRecord, publicDir: string): Promise<PublishedPhoto> {
  ...
  const large = join(publicDir, `${base}-large.webp`),
    medium = join(publicDir, `${base}-medium.webp`),
    thumb = join(publicDir, `${base}-thumb.webp`);
  ...
  return {
    id: photo.id,
    srcLarge: `/trip/generated/${base}-large.webp`,
    srcMedium: `/trip/generated/${base}-medium.webp`,
    srcThumb: `/trip/generated/${base}-thumb.webp`,
