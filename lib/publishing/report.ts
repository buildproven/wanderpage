...
export async function writeReports(
  ...
  await Promise.all([
    ...
    writeFile(
      ...
      JSON.stringify(
        {
          selected: selection.selected.map(p => p.id),
          rejected: selection.rejected.map(p => ({ id: p.id, reason: selection.reasons[p.id] })),
...
function safePhoto(photo: PhotoRecord) {
  return {
    ...
    similarityCluster: photo.similarityCluster,
    semantic: photo.semantic,
    rejectionReasons: photo.rejectionReasons,
    gpsPresent: photo.gps !== undefined,
  };
}
function reportHtml(
  photos: PhotoRecord[],
  ...
  const rows = photos
    .map(
      p =>
        `<tr><td>${p.id}</td><td>${selection.selected.includes(p) ? "Selected" : "Rejected"}</td><td>${p.technical.overall.toFixed(1)}</td><td>${escape(selection.reasons[p.id] ?? "")}</td></tr>`
    )
    .join("");
