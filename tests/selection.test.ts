...
const photo = (id: string, people = false, cluster?: string): PhotoRecord => ({
  id,
  hash: id,
  ...
  semantic: {
    photoId: id,
    containsPeople: people,
    peopleProminence: people ? "prominent" : "none",
    aestheticScore: 80,
    storyScore: 80,
    landmarkValue: 80,
    emotionalValue: 80,
...
describe("selection privacy and diversity", () => {
  it("strictly excludes people and duplicate clusters", () => {
    const photos = [
      ...
      photo("b", false, "x"),
      photo("c", true),
      ...Array.from({ length: 18 }, (_, i) => photo(`n${i}`)),
    ];
    const result = selectPhotos(photos, "exclude", 36);
    expect(result.selected.some(p => p.id === "c")).toBe(false);
