...
describe("trip page names",()=>{
  ...
  it("uses a safe fallback for punctuation-only titles",()=>{expect(tripSlug("— ✈ —")).toBe("untitled-trip");});
  it("updates the same trip while allocating a distinct page for another date",async()=>{
    const workspace=await createTempWorkspace("slugs"),directory=join(workspace,"data/trips"),manifest=TripManifestSchema.parse(rawManifest);
    try{
      await mkdir(directory,{recursive:true});expect(await availableTripSlug(directory,manifest)).toBe("a-line-along-the-pacific");
    ...
    }finally{await removeTempWorkspace(workspace);}
  });
});
