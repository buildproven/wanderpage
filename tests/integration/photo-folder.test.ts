...
describe("photo folder to deployed-site artifact",()=>{
  let workspace="",input="";
  beforeAll(async()=>{workspace=await createTempWorkspace("integration");await copySiteScaffold(workspace);input=await createPhotoFolder(workspace,{heic:true});},30_000);
  afterAll(async()=>{if(workspace)await removeTempWorkspace(workspace);});
  ...
  it("ingests real image files, edits them, builds the static site, and opens it in a browser",async()=>{
    ...
    const result=await runTrip({input,people:"exclude",title:"Controlled Coast Test",maxPhotos:36,privacy:"approximate",force:true,dryRun:false,demo:false},{
      root:workspace,
      aiProvider:provider,
      ...
      enrichDestination:async(destination)=>({
        ...
        sources:[{id:`source-${destination.id}`,title:`${destination.name} fixture source`,url:`https://example.test/${destination.id}`,provider:"Integration fixture"}],
        weather:"10–16 °C",
      }),
    });
    ...
    const server=await staticServer(exported),browser=await chromium.launch({headless:true});
    try{
      ...
      await page.waitForFunction(()=>[...document.querySelectorAll(".gallery-button img")].every((image)=>image instanceof HTMLImageElement&&image.complete&&image.naturalWidth>0));
      const loaded=await page.locator(".gallery-button img").evaluateAll((images)=>images.every((image)=>image instanceof HTMLImageElement&&image.complete&&image.naturalWidth>0));
  ...
  it("runs the documented photo-folder CLI against an isolated workspace",async()=>{
    ...
    try{
      await copySiteScaffold(cliWorkspace);const cliInput=await createPhotoFolder(cliWorkspace,{count:4,gps:false});
...
class RecordingProvider implements AIProvider{
  ...
  async analyzeContactSheet(path:string,photoIds:string[]):Promise<PhotoSemanticAnalysis[]>{
    this.contactSheets.push({path,bytes:(await stat(path)).size});this.photoIds.push(...photoIds);
    return photoIds.map((photoId,index)=>({photoId,containsPeople:index===0,peopleProminence:index===0?"prominent":"none",aestheticScore:82-index,storyScore:78,landmarkValue:70,emotionalValue:45,uniquenessScore:75-index,categories:[index%2?"detail":"landscape"],possibleLocations:[{label:"Oregon Coast",confidence:.7,evidence:"Coastal terrain in the fixture"}],captionSeed:`Visible fixture frame ${index+1}.`}));
  }
  async generateNarrative():Promise<Narrative>{return {title:"Controlled Coast Test",subtitle:"A verified fixture route.",opening:"The controlled photographs follow two coastal groups.",closing:"The fixture route ends with the final selected frame.",chapterNarratives:["A chapter assembled from controlled photo evidence.","A second chapter assembled from controlled photo evidence."],captions:this.photoIds.map((photoId,index)=>({photoId,alt:`Controlled travel photograph ${index+1}`,caption:`Controlled frame ${index+1}.`}))};}
}

async function fixtureDestinations(photos:PhotoRecord[]):Promise<DestinationEvidence[]>{
  const north=photos.filter((photo)=>(photo.gps?.lat??0)>45),south=photos.filter((photo)=>(photo.gps?.lat??99)<=45);
  return [{id:"destination-north",name:"North Coast Fixture",confidence:.96,lat:45.882,lon:-123.962,photoIds:north.map((photo)=>photo.id),evidence:["Controlled GPS cluster"]},{id:"destination-south",name:"South Coast Fixture",confidence:.94,lat:44.642,lon:-124.062,photoIds:south.map((photo)=>photo.id),evidence:["Controlled GPS cluster"]}];
}

async function directoryBytes(path:string):Promise<number>{
  let total=0;for(const entry of await readdir(path,{withFileTypes:true})){const child=join(path,entry.name);total+=entry.isDirectory()?await directoryBytes(child):(await stat(child)).size;}return total;
}

async function fileHashes(root:string,path=root):Promise<Record<string,string>>{
  const hashes:Record<string,string>={};
  for(const entry of await readdir(path,{withFileTypes:true})){const child=join(path,entry.name);if(entry.isDirectory())Object.assign(hashes,await fileHashes(root,child));else hashes[relative(root,child)]=createHash("sha256").update(await readFile(child)).digest("hex");}
...
async function expectTextAbsent(path:string,values:string[]):Promise<void>{
  for(const entry of await readdir(path,{withFileTypes:true})){const child=join(path,entry.name);if(entry.isDirectory())await expectTextAbsent(child,values);else if([".html",".js",".json",".txt",".css",".xml"].includes(extname(child))){const text=await readFile(child,"utf8");for(const value of values)expect(text).not.toContain(value);}}
}

async function staticServer(root:string){
  const server=createServer(async(request,response)=>{
    const requested=decodeURIComponent(new URL(request.url??"/","http://localhost").pathname),relative=requested==="/"?"index.html":requested.replace(/^\/+|\/+$/g,"");
    ...
    try{const metadata=await stat(file);if(!metadata.isFile())throw new Error("Not a file");response.writeHead(200,{"Content-Type":mime(file)});createReadStream(file).pipe(response);}catch{response.writeHead(404).end("Not found");}
  });
  await new Promise<void>((resolve,reject)=>{server.once("error",reject);server.listen(0,"127.0.0.1",resolve);});
  const address=server.address();if(!address||typeof address==="string")throw new Error("Static test server did not bind a TCP port");
...
function mime(path:string){return ({".html":"text/html; charset=utf-8",".js":"text/javascript",".css":"text/css",".json":"application/json",".svg":"image/svg+xml",".webp":"image/webp",".png":"image/png",".jpg":"image/jpeg"} as Record<string,string>)[extname(path)]??"application/octet-stream";}
