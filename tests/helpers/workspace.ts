...
export async function removeTempWorkspace(path:string){
  const systemTemp=await realpath(tmpdir()),resolved=await realpath(path).catch(()=>resolve(path)),parent=dirname(resolved);
...
export async function createPhotoFolder(workspace:string,{count=8,heic=false,gps=true}:{count?:number;heic?:boolean;gps?:boolean}={}){
  const input=join(workspace,"input-photos"),nested=join(input,"day-two");
  ...
  for(let index=0;index<count;index++){
    ...
    const coordinate=index<Math.ceil(count/2)?{lat:45.88+index*.002,lon:-123.96-index*.002}:{lat:44.63+index*.002,lon:-124.05-index*.002};
    await sharp(source)
      .rotate(index%4===3?180:0)
      .modulate({brightness:1+(index%3)*.025,saturation:1+(index%2)*.04})
      .withExif({IFD0:{Model:"Wanderpage Integration Camera"},IFD2:{DateTimeOriginal:`2026:09:${String(index+4).padStart(2,"0")} 12:00:00`},...(gps?{IFD3:gpsExif(coordinate.lat,coordinate.lon)}:{})})
      .jpeg({quality:88})
      .toFile(target);
    paths.push(target);
  }
...
function gpsExif(lat:number,lon:number){
  return {GPSLatitudeRef:lat>=0?"N":"S",GPSLatitude:degrees(Math.abs(lat)),GPSLongitudeRef:lon>=0?"E":"W",GPSLongitude:degrees(Math.abs(lon))};
}

function degrees(value:number){
  const whole=Math.floor(value),minutes=(value-whole)*60,minuteWhole=Math.floor(minutes),seconds=Math.round((minutes-minuteWhole)*6000);
